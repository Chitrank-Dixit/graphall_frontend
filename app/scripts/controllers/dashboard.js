...
app
  .factory('TrackingData', function($http, $q) {
    ...
    return {
        get_tracking_data: function() {
          return $http.post($scope.main.settings.serverUrl +'/api-token-refresh/', token).success(function (data) {
            if (data.token) {
              $http({
                method: 'POST',
                url: $scope.main.settings.serverUrl +'/get_custom_ranged_tracking_source_data/',
  ...
  .controller('DashboardCtrl', function($scope,$http, $window, $state, $timeout){
    ...
    $scope.logoutUser = function() {
      ...
      $http({
        method: 'POST',
        url: $scope.main.settings.serverUrl +'/api/v1/auth/logout/',
        data :authData,
    ...
    $scope.filterBySource = function(tracking_id) {
      ...
      $http.post($scope.main.settings.serverUrl +'/api-token-refresh/', token ).success(function(data) {
        if (data.token) {
          $http({
            method: 'GET',
            url: $scope.main.settings.serverUrl +'/api/v1/tracking_data/?tracking_source_id='+$scope.source.show.source,
            headers: {"Content-Type": "application/json", "Authorization": "JWT "+data.token}  // set the headers so angular passing info as form data (not request payload)
          }).success(function (data) {
